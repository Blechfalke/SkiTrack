...
@Api(name = "trackendpoint", namespace = @ApiNamespace(ownerDomain = "technotracks.ch", ownerName = "technotracks.ch", packagePath = ""))
public class TrackEndpoint {
	 ...
	 */
	@ApiMethod(name = "removeTrack")
	public void removeTrack(@Named("id") Long id) {
		EntityManager mgr = getEntityManager();
		try {
			Track track = mgr.find(Track.class, id);
			mgr.remove(track);
		} finally {
			mgr.close();
		}
